...
#ifdef WLAN_PROV_DEBUG_LOG_ENABLED
...
int main(int argc, char **argv)
{
	...
	if (get_prov_mac_address(&ProvMacAddr) < 0) {
		/* read error*/
		LOG_I("Provisioned MAC not found, request to randomize one");
		...
		LOG_I("Provisioned Mac address:  %02x:%02x:%02x:%02x:%02x:%02x",
	...
	if (ProvMacAddr && (memcmp(ProvMacAddr, NullMacAddr, MAC_ADDRESS_LEN) == 0)) {
		...
		if (cal_check_mac_file_exist() > 0){
			LOG_I("mac address exist, Nothing to do..");
			goto end;
		}

		LOG_I("Mac address doesn't exist, randomize one.");
...
static int cal_create_new_mac_file(unsigned char *MacAddr){
	...
	ret = snprintf(buff, BUFF_SIZE, "%02hhx%02hhx%02hhx%02hhx%02hhx%02hhx\n",
...
static int get_prov_mac_address(unsigned char** macAddr) {
	...
	if (ret != MAC_ADDRESS_LEN) {
		LOG_I("get_customer_data error: 0x%x", ret);
		if (ret >0) {
			free(*macAddr);
			*macAddr = NULL;
		}
	...
	*macAddr = malloc(sizeof(unsigned char) * MAC_ADDRESS_LEN);
	...
	if (ret != 0) {
		LOG_I("tee_token_item_read error: 0x%x", ret);
		free(*macAddr);
		*macAddr = NULL;
		return -1;
	}
...
static int cal_update_mac_file(unsigned char *MacAddr)
{
	...
	if (fp == NULL){
		LOG_I("mac file does not exist, create a new one");
	...
	if (cal_parse_buff(buff, macAddr)) {
		if (memcmp(MacAddr, macAddr, MAC_ADDRESS_LEN) != 0) {
			LOG_I("mac from file is is not aligned, mac has to be updated");
			return cal_create_new_mac_file(MacAddr);
		}
		else {
			LOG_I("mac from file is aligned, nothing to do");
			return 1;
		}
	}

	LOG_I("error on parsing mac, mac has to be updated");
...
static int cal_parse_buff(char *buff, unsigned char *macAddr){
	...
	if (buff == NULL || macAddr == NULL) {
		return -1;
	}
	if( sscanf(buff, "%2hhx%2hhx%2hhx%2hhx%2hhx%2hhx\n", &macAddr[0], &macAddr[1], &macAddr[2], &macAddr[3], &macAddr[4], &macAddr[5]) == 6){
			LOG_I("parsed mac %02x:%02x:%02x:%02x:%02x:%02x", macAddr[0], macAddr[1], macAddr[2], macAddr[3], macAddr[4], macAddr[5]);
			return 1;
	}

	LOG_I("Problems on parsing the mac address");
	return -1;
}
